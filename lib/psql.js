...
module.exports = {
  ...
  getBirdsByUser: userId =>
    db.any('SELECT * FROM birds WHERE id = (SELECT id_bird FROM birds_users_locs WHERE id_user_loc = (SELECT id FROM users_locations WHERE id_users = (SELECT id FROM users where username = $1)))', [userId]),
